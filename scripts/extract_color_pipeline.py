...
def classify_mock_rules(feats, brix_hat):
    hue = float(feats.get("hsv0_mean", 0.0))  # OpenCV Hue: 0..180 ≈ องศา
    a_mean = float(feats.get("lab1_mean", 0.0))

    # ใช้ Hue เป็นตัวชี้นำหลัก (แบ่งช่วงไม่ทับซ้อน)
    if hue <= 30:
        return "Ready", (0, 255, 0)
    if hue > 50:
        return "Below", (0, 0, 255)
    if 30 < hue <= 50:
        return "Wait", (255, 0, 0)

    # กรณีไม่มี hue ที่เชื่อถือได้ ใช้ตาม Brix + a*
...
def main():
    ...
    for p in sorted(glob.glob(os.path.join(IMG_DIR, "*.*"))):
        ...
        for i, m in enumerate(masks):
            ...
            # เก็บไว้ก่อน เดี๋ยว auto-calibrate ทีหลัง (ถ้าเปิด)
            # ใส่ค่าชั่วคราว เดี๋ยวจะอัพเดตอีกครั้งตอนท้ายก็ได้
            brix_hat, _ = proxy_brix_and_class(feats, T_MIN_DEFAULT, T_READY_DEFAULT)
            # จัดกลุ่มสถานะตาม mock rules และกำหนดสีกรอบ
            label, _oldcolor = None, None
    ...
    if AUTO_CAL and brix_list:
        brix_arr = np.array(brix_list)
        t_min   = float(np.percentile(brix_arr, 30))   # ปรับตามสัดส่วนชุดภาพ
        t_ready = float(np.percentile(brix_arr, 70))
        # อัพเดตผลลัพธ์ด้วย thresholds ใหม่
        for r in rows:
            bh = r["brix_hat"]
            if bh >= t_ready: r["class_proxy"] = "Ready"
            elif bh <  t_min: r["class_proxy"] = "Below"
            else:             r["class_proxy"] = "Wait"
    ...
    if len(df):
        ...
        print("Brix_hat stats:", df["brix_hat"].describe().to_dict())
